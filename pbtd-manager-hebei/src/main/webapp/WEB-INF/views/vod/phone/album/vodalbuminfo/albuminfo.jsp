   ...
   a.l-btn span span.l-btn-text {
    ...
    <body class="easyui-layout" style="visibility:hidden">
      <div data-options="region:'north',border:false,split:true" style="padding:0px; border-bottom:1px solid #99BBE8; height:38px; width:auto;">
            ...
            <a href="javascript:getalbuminfor()" id="btnSearch" class="easyui-linkbutton" data-options="iconCls:'icon-search',plain:false,disabled:false">查询</a> 
            </form>
        </div>
        ...
        <script type="text/javascript"> 
            function getalbuminfor() {
                ...
                if(typeof($.data(t[0], 'datagrid')) == 'undefined') {
                	...
                	t.domain('datagrid', {
                        title: '绑定数据',
                    	url: '${pageContext.request.contextPath}/vod/phone/vodalbuminfo/albuminfopage/'+id,
	                    ...
	                    title:"添加节目",
	               		...
	               		handler: function() {   
	               			$(parent).domain("openDialog", { 	               				
	    	     	        	iconCls: "icon-view", 
	    	     	        	title: "添加", 
	    	     	        	src: "${pageContext.request.contextPath}/vod/phone/vodalbuminfo/addalbuminfor/${id}",
	    	     	        	width: 1250,
	    	    	        	height: 650,
	    	    	        	onClose: function() { 
	    	     	        	var ids = top.$.data(top.document.body, "choose.ids");
	    	   	               	 $.ajax({
	    	   	               	     		url:"${pageContext.request.contextPath}/vod/phone/vodalbuminfo/addalbumrecommand",
	    	   	               	     		type:"post",
	    	   	               	     		data:{"id":$.query.getId(),"seriesCode":ids},
	    	   	               	     		dataType:"json",
	    	   	               	         	success:function(){
	    	   	               	         	getalbuminfor();  		
	    	                              	},
	               		...
	               		handler: function() {
	               	        ...
	               	        $.messager.confirm('确认','确认删除？',function(r){
	               	        	if(r==false){return false;}
	    	     	        		$.ajax({
	    	                           	url:"${pageContext.request.contextPath}/vod/phone/vodalbuminfo/deletealbumrecommand",
	    	                           	type:"GET",
	    	                           	dataType:"JSON",
	    	                           	data:{seriesCode:ids.substr(1,ids.length),id:"${id}"},
	    	                           	async:false,
	    	                           	success:function(){
	        				...
	        				nodes = $table.datagrid("getChanges");
	               	        ...
	               	        for (var i = 0; i < nodes.length; i++) {
	            	            ...
	            	            sequence+=","+nodes[i]["sequence"];
	    	     	        		...
	    	     	        		$.ajax({
	    	                           	url:"${pageContext.request.contextPath}/vod/phone/vodalbuminfo/updatealbumrecommand",
	    	                           	type:"GET",
	    	                           	dataType:"JSON",
	    	                           	data:{seriesCode:ids.substr(1,ids.length),id:"${id}",sequence:sequence.substr(1,sequence.length)},
	    	                           	async:false,
	    	                           	success:function(){
	    	                           	 top.$.messager.alert("信息", "操作成功！", "info", null, 2000);            		
	    	                           	},
