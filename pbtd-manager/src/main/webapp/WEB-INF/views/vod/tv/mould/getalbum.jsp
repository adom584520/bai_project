        ...
        <script type="text/javascript"> 
            function getinfo() {
                ...
                if(typeof($.data(t[0], 'datagrid')) == 'undefined') {
                	...
                	t.domain('datagrid', {
                        title: '绑定数据',
                    	url: '${pageContext.request.contextPath}/vod/tv/vodmouldinfo/albuminfopage',
                        ...
                        columns: [[
	                    	...
	                    	,{field: 'seriesCode', title: '唯一标识', width: 80,fit:true,  height: 132,  align: 'center', hidden: false}
	                    	,{field: 'seriesName', title: '名称', width: 100,   align: 'left', hidden: false}
	                    	,{field: 'writerName', title: '导演', width: 80,  align: 'left', hidden: false}
	                    	,{field: 'actorName', title: '演员', width: 120,  align: 'left', hidden: false}
	                    	,{field: 'pictureurl1', title: '专辑图片1', width: 120,  align: 'left', hidden: false,formatter: getimgurl}
	                    	,{field: 'pictureurl2', title: '专辑图片2', width: 120,  align: 'left', hidden: false,formatter: getimgurl}
	                    	,{field: 'ChannelName', title: '频道', width: 50, sortable: true, align: 'center', hidden: false}
	                    	,{field: 'volumncount', title: '总集数', width: 50,  align: 'center', hidden: false}
	                    	,{field: 'currentnum', title: '更新剧集', width: 50,  align: 'center', hidden: false}
	                    	,{field: 'status', title: '状态', width: 50,  align: 'center', hidden: false,formatter:getstatus}
	                    	,{field: 'sequence', title: '排序', width: 50,  align: 'center', hidden: false,
	                    	editor:{
                    			type:"numberbox",
                    			options:{
                    				min:0,   
	                    ...
	                    title:"添加节目",
	               		...
	               		handler: function() {
	               			$(parent).domain("openDialog", { 
	    	     	        	iconCls: "icon-view", 
	    	     	        	title: "添加", 
	    	     	        	src: "${pageContext.request.contextPath}/vod/tv/vodSpecial/addalbuminfo/"+1,
	    	     	        	width: widthnum, 
	    	    	        	height: 550,
	    	     	        	onClose: function() { 
	    	     	        		var ids = top.$.data(top.document.body, "choose.ids");
	    	     	        		$.ajax({
	    	                           	url:"${pageContext.request.contextPath}/vod/tv/vodmouldinfo/addalbuminfo/${curid}/${num}",
	    	                           	type:"GET",
	    	                           	dataType:"JSON",
	    	                           	data:{albumid:ids},
	    	                           	async:false,
	               	    	...
	               	    	$.messager.confirm('确认','确认删除？',function(r){
	               	    		if(r){
			               	        ...
			               	        for (var i = 0; i < nodes.length; i++) {
		   	     	        		...
		   	     	        		$.ajax({
		   	                           	url:"${pageContext.request.contextPath}/vod/tv/vodmouldinfo/deletealbuminfo/${curid}/${num}",
		   	                           	type:"GET",
		   	                           	dataType:"JSON",
	               		...
	               		handler: function() {
	               	    	...
	               	    	$.messager.confirm('确认','确认启用？',function(r){
	               	    		if(r){
	               	    			var ids = nodes[0]["id"];
		   	     	        		$.ajax({
		   	                           	url:"${pageContext.request.contextPath}/vod/tv/vodmouldinfo/shangxianalbuminfo",
		   	                           	type:"GET",
		   	                           	dataType:"JSON",
	        				...
	        				nodes = $table.datagrid("getChanges");
	               	        ...
	               	        for (var i = 0; i < nodes.length; i++) {
	            	            ...
	            	            sequence+=","+nodes[i]["sequence"];
	    	     	        		...
	    	     	        		$.ajax({
	    	                           	url:"${pageContext.request.contextPath}/vod/tv/vodSpecial/updatealbumsequence",
	    	                           	type:"GET",
	    	                           	dataType:"JSON",
	        	 ...
	        	 function  getimgurl(e,row){
	        		 ...
	        		 if(imgStatus==1){
	        			 if(e==row.pictureurl1 || e==row.pictureurl2){
	        				 ...
	        				 //str+=" style='position:relative;width:80px;height:60px;background:rgba(0,0,0,);z-index:-1;'>";	 
	        			 }else{
	        	  ...
	        	  function getBtn(value, row, index, text){
		           		 var ckButton='';
		   		    	 ckButton = "<a href='#' onclick='show(\""+row.id+"\",\"yywimgurl1\")' class='easyui-linkbutton'>图1上传</a>";
		   		    	 ckButton += "&nbsp;&nbsp;&nbsp;<a  href='#' onclick='show(\""+row.id+"\",\"yywimgurl2\")' class='easyui-linkbutton'>图2上传</a>";
		   		    	 ckButton += "&nbsp;&nbsp;&nbsp;<a  href='#' onclick='show(\""+row.id+"\",\"yywimgurl3\")' class='easyui-linkbutton'>图3上传</a>";
		   		    	 ckButton += "&nbsp;&nbsp;&nbsp;<a  href='#' onclick='show(\""+row.id+"\",\"yywimgurl4\")' class='easyui-linkbutton'>图4上传</a>";
		   		    	 return ckButton;
		   	   		}
