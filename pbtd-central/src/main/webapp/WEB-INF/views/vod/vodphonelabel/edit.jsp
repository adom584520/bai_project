...
<div data-options="region:'center',border:false" style="padding:0px;">
	...
	<input type="hidden" id="id" name="id" />
	  <input type="hidden" id="levels" name="levels" value="2"  />
		...
		分类
				...
				<td class="tdContent">
				<select id="type"  name="type"   style="width: 120px;" onchange="load();">
            	</select>
				</td>
			</tr>  
			<tr>
		 ...
		 标识code
				...
				<td class="tdContent">
		<input type="text" id="categorycode"name="categorycode" <c:if test="${id>0 }"> disabled="disabled" </c:if> class="input text easyui-numberbox"  style="" data-options="validType:'text[1,30]',required:true" />	
	</td>
			</tr>
			<tr>
				<td style="text-align: right; width: 15%"  class="tdCaption"  align="center">
...
<script type="text/javascript">
	//页面初始化
   	$(function(){
	       ...
	       //加载数据
	        t.domain('load', {
	            url: '${pageContext.request.contextPath}/integrate/vodphonelabel/load/' + id,
	            names: [],
	    ...
	    var  url= '${pageContext.request.contextPath}/integrate/vodphonelabel/'+($.query.getId()=="0"?'create':'edit/'+id);  
		     t.domain('edit', {
	             title: ' ',
	             url: url,
	             data: data,
    ...
    function load() {
      	var type=$("#type").val();
  	$.ajax({
        url: '${pageContext.request.contextPath}/integrate/vodphonelabel/loadtype/' + type,
       	type:"GET",
       	dataType:"JSON",
       	data:{},
       	async:false,
       	success:function(msg){
       		$("#categorycode").val(msg);
       	},
      ...
      //选择分类
    	function choosechannel(){
               	$.ajax({
               	  	url:"${pageContext.request.contextPath}/pbtd/dictionary/chooselabeltype",
                   	type:"GET",
                   	dataType:"JSON",
                   	data:{},
                   	async:false,
