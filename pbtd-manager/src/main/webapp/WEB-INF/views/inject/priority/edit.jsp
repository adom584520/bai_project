...
<div data-options="region:'center',border:false" style="padding:0px;">
	...
	<input type="hidden" id="id" name="id" />
		...
		频道名称
				...
				<select id="channel_name" name="channel_name"  style="width: 120px;" class="chzn-select">
						<c:forEach var="map" items="${channelNamelist}">
							<option value="${map.channel_name}"<c:if test="${id >0 }">disabled="disabled"</c:if>  
								<c:if test="${channel_name eq  map.channel_name}"></c:if>
							>
									${map.channel_name}
							</option>
						</c:forEach>
...
<script type="text/javascript">
	//页面初始化
   	$(function(){
	        ...
	        //加载数据
	        t.domain('load', {
	            url: '${pageContext.request.contextPath}/inject/priority/load/' + id,
	            names: [],
	    ...
	    var  url= '${pageContext.request.contextPath}/inject/priority/'+($.query.getId()=="0"?'create':'edit/'+id);  
		     t.domain('edit', {
	             title: ' ',
	             url: url,
	             data: data,
