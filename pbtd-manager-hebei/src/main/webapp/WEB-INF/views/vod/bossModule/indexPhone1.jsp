...
<script type="text/javascript">
	...
	function childUpDownFunctionPhone(channelname, channel, upmoduleSequence) {
		//console.log("channel:"+channel);
		channel = channel.substr(channel.indexOf('_')+1, channel.length);
		//console.log("channel:"+channel);
		var obj = channelname+"本地预览";
		window.parent.addleftText(obj);
		...
		if (channel == $(".module").attr("channle")) {
			$(".module>h3").each(function(i, ele) {
				...
				if (moduleSequence === sequence) {
					//console.log(moduleSequence);
					var scrolltop = $(ele).offset().top-30;
	...
	function edit(obj) {
		...
		$(".bg").show();
		var url = "${pageContext.request.contextPath}/vod/phone/vodmouldinfo/getalbum/"+ mId + "/" + num;
		...
		$(window).domain( "openDialog", {
				iconCls : "icon-view",
				title : "绑定",
				src : "${pageContext.request.contextPath}/vod/phone/vodmouldinfo/getalbum/"+ mId + "/" + num,
				width : widthnum - 20,
				height : 550,
				top : scrolltop,
				onClose : function() {
								var url = "${pageContext.request.contextPath}/vod/phone/vodmouldinfo/getchannelflag";
								$.ajax({
									type : "post",
									url : url,
									data : {"moduleId" : mId},
