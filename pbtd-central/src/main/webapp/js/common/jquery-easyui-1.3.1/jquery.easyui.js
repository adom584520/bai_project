					...
					if (_e0.onlyLeafCheck) {
						if (_e8.state == "open" && (!_e8.children || !_e8.children.length)) {
							if (_e8.checked) {
								...
								$("<span class=\"" + (_e8.id ? "tree-checkbox tree-checkbox1" : "") + "\"></span>").prependTo(_e9);
							} else {
								//$("<span class=\"tree-checkbox tree-checkbox0\"></span>").prependTo(_e9);
								$("<span class=\"" + (_e8.id ? "tree-checkbox tree-checkbox0" : "") + "\"></span>").prependTo(_e9);
							}
						}
					} else {
						if (_e8.checked) {
...
(function($) {
	...
	;
	function _222(_223, _224, _225, timeout) {//add timeout
	...
	$.messager = {
		...
		alert : function(_228, msg, icon, fn, timeout) {//add timeout
			...
			var win = _222(_228, _229, _22a, timeout);//add timeout
			return win;
		},
			...
			cc.children("div").each(
					function() {
						...
						if (r == "north" || r == "south" || r == "east" || r == "west" || r == "center") {
							...
							//add
							_2f4(_2f1, $.extend({}, {marginTop: 0, marginLeft: 0, marginBottom: 0, marginRight: 0}, opts), this);
						}
					});
		}
		;
...
(function($) {
	...
	var _5af = {
		...
		renderRow : function(_5be, _5bf, _5c0, _5c1, _5c2, textRow) {//add textRow
			...
			for ( var i = 0; i < _5bf.length; i++) {
				...
				if (col) {
					...
					var _5c6 = col.styler ? (col.styler(_5c5, _5c2, _5c1, text) || "") : "";// add text
					...
					if (col.checkbox) {
						...
						if (col.formatter) {
							cc.push(col.formatter(_5c5, _5c2, _5c1, text));//add text
						} else {
							//delete
							//cc.push(_5c5);
							//add
		...
		updateRow : function(_5ca, _5cb, row) {
			...
			var _5cc = opts.rowStyler ? opts.rowStyler.call(_5ca, _5cb, rows[_5cb], textRow) : "";//add textRow
			function _5cd(_5ce) {
				...
				tr.html(this.renderRow.call(this, _5ca, _5cf, _5ce, _5cb, rows[_5cb], textRow));//add textRow
				tr.attr("style", _5cc || "");
				if (_5d0) {
...
(function($) {
	...
	;
	function _698(_699, _69a, flag) {//add flag:为true时表明是reload方法调用的
		...
		if (_69b.length) {
			...
			_656(_699, row[opts.idField], /*{id : row[opts.idField]}*/queryParams, true, function() {
				if (cc.is(":empty")) {
					_69b.remove();
					//start:add
					if(!flag) {
	...
	$.fn.treegrid.methods = {
		...
		reload : function(jq, param) {//update:id to param
			return jq.each(function() {
				/*
				if (id) {
				...
				*/
				if(typeof(param) == "undefined" || param == null) {
					...
					_656(this, null, queryParams);
					//end
				}
				else {
					if(typeof(param) == "object") {
						_656(this, param.id, param);
					}
					else {
						var id = param;
						var node = $(this).treegrid("find", id);
			...
			.extend(
					...
					$.fn.datagrid.defaults.view,
					{
						render : function(_6c7, _6c8, _6c9) {
							...
							var view = this;
							var _6cc = _6cd(_6c9, this.treeLevel, this.treeNodes, this.treeNodesOfText);//add this.treeNodesOfText
							$(_6c8).append(_6cc.join(""));
							function _6cd(_6ce, _6cf, _6d0, treeNodesOfText) {//add treeNodesOfText
								...
								for ( var i = 0; i < _6d0.length; i++) {
									...
									var _6d2 = opts.rowStyler ? opts.rowStyler.call(_6c7, row, textRow) : "";//add textRow
									...
									_6d1 = _6d1.concat(view.renderRow.call(view, _6c7, _6ca, _6ce, _6cf, row, textRow));//add textRow
									_6d1.push("</tr>");
						...
						renderRow : function(_6da, _6db, _6dc, _6dd, row, textRow) {//add textRow
							...
							for ( var i = 0; i < _6db.length; i++) {
								...
								if (col) {
									//add
									var text = textRow[_6de] || row[_6de];
									var _6df = col.styler ? (col.styler(row[_6de], row, text) || "") : "";//add text
									...
									if (col.checkbox) {
										...
										var val = null;
										if (col.formatter) {
											val = col.formatter(row[_6de], row, text);//add text
										} else {
											//delete
											//val = row[_6de];
											//add
						...
						updateRow : function(_6e2, id, row) {
							...
							var _6e5 = opts.rowStyler ? opts.rowStyler.call(_6e2, _6e3, textRow) : "";//add textRow
							function _6e6(_6e7) {
								...
								tr.html(this.renderRow(_6e2, _6e8, _6e7, _6e4, _6e3, textRow));//add textRow
								tr.attr("style", _6e5 || "");
...
(function($) {
	...
	function _760(_765, _766, _767) {
		...
		for ( var i = 0; i < _766.length; i++) {
			...
			*/
			//end
			//sratt:add
			var v = $.splitValueAndText(_766[i], 1);
			var s = $.splitValueAndText(_766[i], 2);
			//end
	...
	function _76b(_76c, data, _76d) {
        ...
        var items = [];
        if (opts.nullable) {
        	//$("<div class=\"combobox-item\" value=\"\"></div>").appendTo(_76e);
        ...
        for(var i = 0; i < data.length; i++) {
			...
			var f = opts.formatter ? opts.formatter.call(_76c, data[i]) : s;
			var item = "<div class=\"combobox-item\" value=\"" + v + "\">" + f + "</div>";
			items.push(item);
			if (data[i]["selected"]) {
				(function() {
	...
	$.fn.combobox.parseOptions = function(_782) {
		var t = $(_782);
		return $.extend({}, $.fn.combo.parseOptions(_782), $.parser.parseOptions(_782, [{cache: "boolean"}/*add cache*/, "valueField", "textField", "mode", "method", "url" ]));
	};
...
(function($) {
		...
		function _7be() {
			...
			_7bd.calendar.calendar({
				...
				onSelect : function(date) {
					...
					//add
					var _7c0 = $.date.format(date, "yyyy-MM-dd HH:mm:ss");
					_7c4(_7bc, _7c0);
	...
	function _7c5(_7c6) {
		...
		var _7c7 = $.date.format(c.calendar("options").current, "yyyy-MM-dd HH:mm:ss");
		_7c4(_7c6, _7c7);
	...
	function _7c4(_7c8, _7c9) {
		...
		*/
		...
		var value = "";
		if(_7c9 && $.trim(_7c9)) {
			var formats = ["yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd HH:mm", "yyyy-MM-dd"];
			for(var i = 0; i < formats.length; i++) {
				date = $.date.parse(_7c9, formats[i]);
				if(date != null) {
					break;
				}
			}
		...
		if(date != null) {
			...
			text = $.date.format(date, opts.format);
		}
		else {
			date = new Date();
			date.setHours(0);
		...
		$(_7c8).combo("setValue", value).combo("setText", text);
		_7ca.calendar.calendar("moveTo", date);
		//end
	}
	;
