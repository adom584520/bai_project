...
public class ServerDealService {
	 ...
	 * 其他：失败  
	 */
	public void dealResponseResult(int cmdResult,String correlateId,String resultFileUrl){
		...
		if(cmdResult==0){
			...
			seriesDetail.setCallBackTime(new Date());
			this.seriesDetailMapper.updateStateForCdnReturn(seriesDetail);
			logger.info("okok：CDN返回[注入成功],已经标记字段");
			try{
				//J:获取地址 
				Integer id=muService.getIdFromFtpUrl(resultFileUrl);
				String playUrl=muService.getMovieUrlById(id);
				...
				if(playUrl.indexOf("http")!=-1){
					requestSubWebInjectInterface(seriesDetailForProgramId,1); //成功
					boolean flag=requestZrInterface(seriesDetailForProgramId);
					if(flag){
			...
			this.seriesDetailMapper.updateStateForCdnReturn(seriesDetail);
			logger.info("nono：CDN返回[注入失败],已经标记字段");
	 ...
	 * @throws SQLException 
	 */
	public boolean requestZrInterface(SeriesDetail seriesDetail) throws SQLException{
		/**
		...
		*/
		Integer defBitId=4;
		Integer bitId=seriesDetail.getVersion();
		if(bitId!=null){
			defBitId=bitId;
		}
		...
		String dispatchUrl=dispatcherHostUrl+"/"+seriesDetail.getSeriesId()+"/"
						+seriesDetail.getMovieCode()+"/"+defBitId;
	 ...
	 * @param seriesDetail
	 */
	public void requestSubWebInjectInterface(SeriesDetail seriesDetail,Integer injectState){
		boolean flag=false;
		Map<String,Object> params=new HashMap<String,Object>();
		Integer terminalType=seriesDetail.getTerminalType();
		...
		params.put("injectState", injectState);
		String charset="utf-8";
	...
	public void setValueToRedis(String playUrl,SeriesDetail seriesDetail){
		...
		*/
		...
		String key="vod_"+redisKeyProject+"_1_"+seriesDetail.getSeriesId()+"_"+seriesDetail.getMovieCode()+"_"+defBitId;
