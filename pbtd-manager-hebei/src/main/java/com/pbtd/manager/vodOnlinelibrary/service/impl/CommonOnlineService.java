...
public class CommonOnlineService implements ICommonOnlineService {
	...
	@Override
	public int restmasterplateSon(Map<String, Object> map) {
		map.put("updatetime", curtime);
		List<Map<String, Object>> list=resetmpper.restmasterplateSon(map);
		if(list.size()>0){
			commonMapper.deletesmasterplateSon(map);
			for (Map<String, Object> curmap : list) {
					commonMapper.insertmasterplateSon(curmap);
			}
		}
		return 1;
	}
	...
	@Override
	public int restSysParam(Map<String, Object> map) {
		map.put("updatetime", curtime);
		
		List<Map<String, Object>> list=sysParamMapper.findonline(map);
		if(list.size()>0){
			commonMapper.deletesSysParam(map);
			for (Map<String, Object> curmap : list) {
					commonMapper.insertSysParam(curmap);
			}
		}
		return 1;
	}
