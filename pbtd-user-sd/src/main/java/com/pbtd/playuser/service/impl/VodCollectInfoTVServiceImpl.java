...
import com.pbtd.playuser.mapper.VodCollectInfoTVMapper;
import com.pbtd.playuser.page.CollectHistoryQuery;
import com.pbtd.playuser.service.VodCollectInfoTVService;

@Service
public class VodCollectInfoTVServiceImpl implements VodCollectInfoTVService {
	@Autowired
	private VodCollectInfoTVMapper vodCollectInfoMapper;

	@Transactional
	public void insert(VodCollectInfo collectInfo) {
		vodCollectInfoMapper.delete(collectInfo.getMac(), collectInfo.getSeriesCode());
		collectInfo.setCreateTime(new Date());
		vodCollectInfoMapper.insert(collectInfo);
	}

	@Transactional
	public void delete(String mac, String seriesCode) {
		if (seriesCode != null && seriesCode.length() > 0) {
			...
			if (seriesCode.indexOf(separator) != -1) {
				...
				// 批量删除
				vodCollectInfoMapper.deleteBatch(mac, list);
				return;
			}
			// 删除单条记录
			vodCollectInfoMapper.delete(mac, Integer.valueOf(seriesCode));
			return;
		}
		// 删除用户所有记录
		vodCollectInfoMapper.delete(mac,null);
		return;
	}
	...
	@Override
	public boolean queryIsCollect(String mac, Integer seriesCode) {
		return vodCollectInfoMapper.queryIsCollect(mac, seriesCode) < 1L ? false : true;
	}
