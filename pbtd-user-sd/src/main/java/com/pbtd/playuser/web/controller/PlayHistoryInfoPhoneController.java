...
public class PlayHistoryInfoPhoneController {
	...
	@RequestMapping(value = "/insertPlayHistoryInfo", params = { "userId", "seriesCode", "dramaname"})
	public String insert(final PlayHistoryInfo ph) {
		JSONObject json = new JSONObject();
		try {
			ph.setDramaname(URLDecoder.decode(ph.getDramaname(), "utf-8"));
			ph.setUserId(URLDecoder.decode(ph.getUserId(), "utf-8"));
			...
			json.put("code", 0);
			logger.error("用户系统-点播添加播放记录接口-手机-添加失败！", e);
		}
		new Thread() {
			public void run() {
