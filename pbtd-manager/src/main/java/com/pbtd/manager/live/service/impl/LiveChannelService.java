...
public class LiveChannelService implements ILiveChannelService {
	...
	@Override
	public List<LiveChannel> getchannel(String id) {
		int bussid = liveBussInfoMapper.selectByValue(id).getBussid();
		return this.liveChannelMapper.getchannel(bussid);
	}
	...
	public int updatezt(int id,int status) {
		...
		return liveChannelMapper.updatezt(map);
	}


	@Override
	public int updateimg(LiveChannel liveVersion) {
		return 	liveChannelMapper.updateimg(liveVersion);
	}
}
