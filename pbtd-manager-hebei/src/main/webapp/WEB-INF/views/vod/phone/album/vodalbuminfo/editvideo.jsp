...
button:active {
    ...
    <body class="easyui-layout" style="visibility:hidden">
      <div data-options="region:'north',border:false,split:true" style="padding:0px; border-bottom:1px solid #99BBE8; height:78px; width:auto;">
        ...
        <script type="text/javascript"> 
            function getinfo() {
                ...
                if(typeof($.data(t[0], 'datagrid')) == 'undefined') {
                	...
                	var customToolbar = [$.fn.domain.create,'-',$.fn.domain.edit,'-',sys1Btn,'-',sys2Btn,'-',sys2Btn1,sys3Btn,'-',sys3Btn1,'-',sys4Btn,'-',sys4Btn1];
                	t.domain('datagrid', {
                        title: '剧集信息',
                        ...
                        columns: [[
	                    	...
	                    	,{field: 'cpvideoId', title: '优酷状态', width: 100,  align: 'center', hidden: false,formatter: youkuvideo}
	                    	...
	                    	/* ,{field: 'isCollectfees', title: ' cp源是否收费', width: 100,  align: 'center', hidden: false,
	                    		formatter: function(value) { 
	                    			 if(value=="1"){
	                    				 return '是';
	                    			 }else{
	                    	...
	                    	,{field: 'videopaid', title: ' 己方是否收费', width: 100,  align: 'center', hidden: false,
	                    		formatter: function(value) { 
	                    			 if(value=="1"){
	                    				 return '是';
	                    			 }else{
	                    	...
	                    	,{field: 'cpvId', title: 'cp源标识', width: 120,  align: 'center', hidden: false,formatter: youkuvid}
	                    ]],
        	...
        	$(function(){
        	 ...
        	 function youkuvid(value, row, index, text){
            	 return  row.cpvideoId;
        	 }
        	 function youkuvideo(value, row, index, text){
            	 var youkustatus = $('#youkustatus').val();
        		 if(youkustatus != 0 && value!="" && value!=null){
        			var m = "";
            		var n = "";
			       	 $.ajax({
			             	url:"${ccmp_url}/jsonYouku/somp_getvideo/"+value,
			             	type:"GET",
			         		dataType:"JSON",
			         		timeout : 3000,
			         		async:false,
        	...
        	$.fn.domain.edit={
	        	 ...
	        	 //己方收费 是
	        	 var  sys4Btn=$.extend($.fn.domain.Sysbtn41,{
	             	title:"收费",
	            	text:"收费 ",
	        		handler: function() {
	        			var nodes =  $table.datagrid("getSelections");
	        	    	if (!nodes || nodes.length == 0) {
		           	     ...
		           	     updatevideopaid(ids.substr(1,ids.length),1);
	                },
	                scope:"one,more"
	        	});
	        	 //显示
	        	 var  sys4Btn1=$.extend($.fn.domain.Sysbtn42,{
	             	title:"不收费",
	            	text:"不收费",
	        		handler: function() {
	        			var nodes =  $table.datagrid("getSelections");
	        	    	if (!nodes || nodes.length == 0) {
		           	     ...
		           	     updatevideopaid(ids.substr(1,ids.length),0);
	                },
	                scope:"one,more"
	        	});
	        	 function updatevideopaid(ids,type){
	        		 var  url= '${pageContext.request.contextPath}/vod/phone/vodalbuminfo/video/updatevideopaid';	  	 
		  			 $.ajax({
		               	url:url,
		               	type:"POST",
		               	data:{id:ids,videopaid:type},
		               	dataType:"JSON",
		               	success:function(){
		               			$.messager.alert("信息","修改成功！","info",function(){
		               				$table.datagrid("load");
