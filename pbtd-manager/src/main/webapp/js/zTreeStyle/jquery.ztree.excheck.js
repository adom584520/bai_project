...
(function($){
	...
	_eventProxy = function(e) {
		...
		if (tools.eqs(e.type, "mouseover")) {
			...
			//子集单击事件
			var son=target.getAttribute("son");
			var curid=target.getAttribute('curid');
			var curpid=target.getAttribute("curpid");
			var clitype=target.getAttribute("clitype");
			var cursequence=target.getAttribute("cursequence");
			var channelname = target.innerText.replace("*","");
			if(curpid == ''){
				 var url = "/vod/tv/vodmouldinfo/basePage?channel="+curid+"&channelname="+channelname;
					$("#iframe").attr("src",url); 
			}
			//TODO
			if(son==1 && curpid!=0 && clitype == "tv"){
			// 	return    $('#con_right_left').load('/vod/tv/vodmouldinfo/getmaster/'+curid);
				///$('#con_right_left').load('/vod/tv/vodmouldinfo/yulanmouldbyid/1/' + curid);
				/*return	$('#con_right_right').load('/vod/tv/vodmouldinfo/yulanmouldbyid/2/' + curid);*/
			//	alert(target.getAttribute("clitype"));
			//	alert(target.getAttribute("cursequence"));
				document.getElementById("iframe").contentWindow.childUpDownFunction(channelname,curpid,cursequence+"");
			}else if(son==1 && curpid!=0 && clitype == "phone"){
				//return    $('#con_right_left').load('/vod/phone/vodmouldinfo/getmaster/'+curid);
				/*$('#con_right_left').load('/vod/phone/vodmouldinfo/yulanmouldbyid/1/' + curid);
				return	$('#con_right_right').load('/vod/tv/vodmouldinfo/yulanmouldbyid/2/' + curid);*/
				document.getElementById("iframe").contentWindow.childUpDownFunction(channelname,curpid,cursequence+"");
			}else{
