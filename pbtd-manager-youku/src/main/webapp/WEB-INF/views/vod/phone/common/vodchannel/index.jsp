...
<body class="easyui-layout" style="visibility: hidden">
	<div data-options="region:'north',border:false,split:true"
		style="padding: 0px; border-bottom: 1px solid #99BBE8; height: 38px; width: auto;">
		<form id="formQuery" style="margin: 0; padding: 0" action=""
			method="post">
			<span class="property"> <label class="">频道：</label> <select
				id="parentCode" name="parentCode" style="width: 120px;"
				class="chzn-select">
				...
				style="width: 200px;" />
			</span> <span class="property"> <label class="">层级：</label> <select
				id="levels" name="levels" style="width: 80px;" class="chzn-select">
					<option value="">--请选择--</option>
					<option value="1">1</option>
					<option value="2">2</option>
					<option value="3">3</option>
			</select>
			</span> <span class="property"> <label class="">状态：</label> <select
				id="status" name="status" style="width: 80px;" class="chzn-select">
					<option value="">--请选择--</option>
					<option value="0">下线</option>
					<option value="1">上线</option>
			</select>
			</span> <a href="javascript:getinfo()" id="btnSearch"
	...
	<script type="text/javascript">
		function getinfo() {
			...
			if (typeof ($.data(t[0], 'datagrid')) == 'undefined') {
				...
				var customToolbar = [ $.fn.domain.create, '-',
						$.fn.domain.edit, '-', sysBtndel, '-', sysBtnup, '-',
						sysBtnno, '-', sysBtn, '-', sysBtnbq, '-', sysBtnreset,
						sysBtns6, '-' ];
				t
						.domain(
								'datagrid',
								{
									title : '频道管理',
									url : '${pageContext.request.contextPath}/vod/phone/Vodchannel/page',
		...
		var sysBtnup = $.extend($.fn.domain.btnup, {
			...
			handler : function() {
				...
				if (!nodes || nodes.length == 0) {
					top.$.messager.alert("信息", "请您选择需要上线的记录", "info", null,
							2000);
					return;
				}
				var ids = "";
		...
		var sysBtnno = $.extend($.fn.domain.btnno, {
			...
			handler : function() {
				var nodes = $table.datagrid("getSelections");
				if (!nodes || nodes.length == 0) {
					top.$.messager.alert("信息", "请您选择需要下线的记录", "info", null,
							2000);
					return;
				}
				var ids = "";
		...
		function udpatezt(ids, status) {
			$
					.ajax({
						url : "${pageContext.request.contextPath}/vod/phone/Vodchannel/updateStatus",
						type : "GET",
						dataType : "JSON",
						data : {
							status : status,
						...
						async : false,
						success : function() {
							top.$.messager.alert("信息", "操作成功！", "info", null,
									2000);
				...
				.extend(
						$.fn.domain.Btnsys,
						{
							...
							handler : function() {
								var nodes = $table.datagrid("getSelections");
								if (!nodes || nodes.length == 0) {
									top.$.messager.alert("信息",
											"请您选择一条信息进行绑定专辑", "info", null,
											2000);
									return;
								}
								if (nodes.length > 1) {
								...
								if (levels == 2) {
									if (nodes[0]["name"] == ""
											|| nodes[0]["name"] == null
											|| nodes[0]["parentCode"] == ""
											|| nodes[0]["parentCode"] == null) {
										top.$.messager.alert("信息",
												"请先编辑归属频道再绑定专辑", "info", null,
												2000);
										return;
									}
								...
								$(parent)
										.domain(
												"openDialog",
												{
													iconCls : "icon-view",
													title : "绑定",
													src : "${pageContext.request.contextPath}/vod/phone/Vodchannel/albuminfo/"
															+ id
															+ "?parentCode="
															+ parentCode
															+ "&channelName="
				...
				.extend(
						$.fn.domain.reset,
						{
							...
							handler : function() {
								$
										.ajax({
											url : "${pageContext.request.contextPath}/phonereset/resetchannel",
											type : "GET",
											dataType : "JSON",
											data : {},
											async : false,
				...
				.extend(
						$.fn.domain.Btnsys6,
						{
							...
							handler : function() {
								var nodes = $table.datagrid("getSelections");
								if (!nodes || nodes.length == 0) {
									top.$.messager.alert("信息", "请您选择要同步的专辑",
											"info", null, 2000);
									return;
								}
								var ids = "";
				...
				.extend(
						$.fn.domain.del,
						{
							...
							handler : function() {
								var nodes = $table.datagrid("getSelections");
								if (!nodes || nodes.length == 0) {
									top.$.messager.alert("信息", "请您选择需要删除的记录",
											"info", null, 2000);
									return;
								}
								var status = "";
								...
								for (var i = 0; i < nodes.length; i++) {
									...
									if (status == 1) {
										top.$.messager.alert("信息", "上线记录不允许删除",
												"info", null, 2000);
										return;
									}
								}
								$.messager
										.confirm(
												'确认',
												'请确认数据是否已同步，删除后数据无法更新线上已有数据，请核对后谨慎删除，确认删除？',
												function(r) {
													if (r == false) {
														return false;
													}
													$
															.ajax({
																url : "${pageContext.request.contextPath}/vod/phone/Vodchannel/deletes",
																type : "GET",
																dataType : "JSON",
																data : {
																	id : ids
									...
									$(parent)
											.domain(
													"openDialog",
													{
														iconCls : "icon-view",
														title : "绑定",
														src : "${pageContext.request.contextPath}/vod/phone/Vodchannel/getchannellabel/"
																+ id
																+ "?channelName="
																+ channelName
																+ "&levels="
			...
			$(parent)
					.domain(
							"openDialog",
							{
								iconCls : "icon-view",
								title : "上传",
								src : "${pageContext.request.contextPath}/uploadPic/updateimg?picname=vodphonechannelimg&id="
										+ id + "&imgtype=" + imgtype,
								width : 450,
								height : 250,
								onClose : function() {
