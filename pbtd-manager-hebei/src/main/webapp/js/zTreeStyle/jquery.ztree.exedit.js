...
(function($){
	...
	_view = {
		addEditBtn: function(setting, node) {
			...
			editStr = "<span class='" + consts.className.BUTTON + " edit'  style='margin-left:10px;' id='" + node.tId + consts.id.EDIT + "' title='"+tools.apply(setting.edit.renameTitle, [setting.treeId, node], setting.edit.renameTitle)+"' treeNode"+consts.id.EDIT+" style='display:none;'></span>";
			aObj.append(editStr);

			$$(node, consts.id.EDIT, setting).bind('click',
				function() {
				if(confirm("确认编辑"+node.name+"频道？")){
		...
		addRemoveBtn: function(setting, node) {
			...
			var aObj = $$(node, consts.id.A, setting),
			removeStr = "<span class='" + consts.className.BUTTON + " remove'  style='margin-left:10px;' id='" + node.tId + consts.id.REMOVE + "' title='"+tools.apply(setting.edit.removeTitle, [setting.treeId, node], setting.edit.removeTitle)+"' treeNode"+consts.id.REMOVE+" style='display:none;'></span>";
		...
		addYulanBtn: function(setting, node) {
			 // alert("进入预览");
			if (node.editNameFlag || $$(node, consts.id.YULAN, setting).length > 0) {
				return;
			}
			var aObj = $$(node, consts.id.A, setting),
			yulanStr = "<span class='" + consts.className.BUTTON + " yulan'  style='margin-left:10px;' id='" + node.tId + consts.id.YULAN + "' title='"+tools.apply(setting.edit.yulanTitle, [setting.treeId, node], setting.edit.yulanTitle)+"' treeNode"+consts.id.YULAN+" style='display:none;'></span>"
			aObj.append(yulanStr);
			$$(node, consts.id.YULAN, setting).bind('click',
				function() {
				//TODO 预览的方法改成：同步模块的信息
				document.getElementById("iframe").contentWindow.childUpDownFunction(node.name,node.pId,node.sequence+"");
				return false;
				}
				).bind('mousedown',
				function(eventMouseDown) {
		...
		removeYulanBtn: function(setting, node) {
			$$(node, consts.id.YULAN, setting).unbind().remove();
		},
		removeTreeDom: function(setting, node) {
			node.isHover = false;
			view.removeEditBtn(setting, node);
			view.removeRemoveBtn(setting, node);
			view.removeYulanBtn(setting, node);
