...
import com.pbtd.playuser.web.controller.PlayHistoryInfoPhoneController;
...
public class RequestUtil {
	 ...
	 * @return
	 */
	public static Map<String, Object> asMaptoLowerCase(HttpServletRequest request, boolean emptiable) {
		Map<String, Object> params = new HashMap<String, Object>();
		Enumeration names = request.getParameterNames();
		while (names.hasMoreElements()) {
			String name = ((String) names.nextElement()).trim();
			...
			String[] values = request.getParameterValues(name);
			String key = name.replaceAll("\\[([^\\]]*)\\]", "_$1").toLowerCase();
			if (values.length == 1) {
				key = key.replaceAll("_$", "");
				String val = values[0].trim();
				if (val.length() > 0) {
		...
		for(Entry<String, Object> entry : params.entrySet()) {
			logger.info("入参===>"+entry.getKey()+" : "+entry.getValue());
		}
		return params;
	}
