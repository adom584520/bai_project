        ...
        <script type="text/javascript"> 
            function getinfo() {
                ...
                //	var customToolbar = [$.fn.domain.create,'-',$.fn.domain.edit,'-',$.fn.domain.del,'-',saveBtn,'-',savestatusstartBtn,'-',savestatusendBtn];
                	var customToolbar = [$.fn.domain.create,'-',$.fn.domain.edit,'-',sysBtndel,'-',sysBtnup,'-',sysBtnno,'-',sysBtn,'-',sysBtnreset,sysBtns6,'-'];
                	t.domain('datagrid', {
                        title: '专题管理',
                    	url: '${pageContext.request.contextPath}/vod/phone/vodSpecial/page',
                        ...
                        columns: [[
	                    	...
	                    	,{field: 'backgroundimg', title: '背景图', width: 100, sortable: true, align: 'center', hidden: false,formatter: getbackgroundimg}
	                    	...
	                    	,{field: 'textcolor', title: '文字颜色', width: 100, sortable: true, align: 'center', hidden: false	,formatter: function(value) { 
								 if(value=="0"){
		                   				return "<span>黑色</span>";
		                   			 }else{
		                   				 return "<span>白色</span>";
        	...
        	$(function(){
     	    ...
     	    function  getbackgroundimg(e){
     		 ...
     		 function show(id,imgtype){
	         		$(parent).domain("openDialog", { 
	     	        	iconCls: "icon-view", 
	     	        	title: "上传", 
	     	        	src: "${pageContext.request.contextPath}/uploadPic/updateimg?picname=vodSpecial&id="+id+"&imgtype="+imgtype,
	     	        	width: 450, 
	    	        	height: 250,
	     	        	onClose: function() { 
	     	        		getinfo();
        	 ...
        	 function udpatezt(ids,zt){
        		  	$.ajax({
                       	url:"${pageContext.request.contextPath}/vod/phone/vodSpecial/editzt",
                       	type:"GET",
                       	dataType:"JSON",
        	...
        	//绑定专辑
        	 var  sysBtn=$.extend($.fn.domain.Btnsys,{
        		...
        		handler: function() {
        	    	...
        	    	$(parent).domain("openDialog", { 
	     	        	iconCls: "icon-view", 
	     	        	title: "绑定", 
	     	        	src: "${pageContext.request.contextPath}/vod/phone/vodSpecial/albuminfo/"+id,
	     	        	width: 1250, 
	    	        	height: 650,
	     	        	onClose: function() {
        	 ...
        	 //当天更新同步
	          	var sysBtnreset=$.extend($.fn.domain.reset,{
	          		...
	          		handler:function(){
	          			$.ajax({
	                        	url:"${pageContext.request.contextPath}/phonereset/resetspecial",
	                        	type:"GET",
	                        	dataType:"JSON",
	                        	data:{},
	                        	async:false,
	        	...
	        	var sysBtndel=$.extend($.fn.domain.del,{
	        		...
	        		handler:function(){
	     			  ...
	     			  $.messager.confirm('确认','请确认数据是否已同步，删除后数据无法更新线上已有数据，请核对后谨慎删除，确认删除？',function(r){
				           	if(r==false){return false;}
				           	$.ajax({
			                  	url:"${pageContext.request.contextPath}/vod/phone/vodSpecial/deletes",
			                  	type:"GET",
			                  	dataType:"JSON",
