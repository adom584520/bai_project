...
public class LogAop {
	...
	public void after(JoinPoint joinPoint, Object ret) {
		...
		// 设置响应参数
		sysLogger.setReturnData(JSON.toJSONString(ret, SerializerFeature.DisableCircularReferenceDetect,
				SerializerFeature.WriteMapNullValue));
		// 执行将日志写入数据库
		try {
			if (sysLogger.getReturnData() != null && sysLogger.getReturnData().length() > 1020) {
