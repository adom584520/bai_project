...
public class OpTemplateServiceImpl implements OpTemplateService {
	...
	public void delete(Long id) {
		...
		ids.add(id);
		operationPositionService.updateDataStatus(ids, LauncherConstant.LAUNCHER_DATA_STATUS_INVALID);
	}
	...
	public void uplineOrDownLine(Long id, Integer status) {
		if (LauncherConstant.UPLINE_STATUS.equals(status)) {
			...
			// 将所有模板所属的导航下所有运营位全部改为下线状态
			operationPositionService.updateStatusByNavId(op.getNavId(), LauncherConstant.DOWNLINE_STATUS);
		}
	...
	public void updateDataStatus(List<Long> list, Integer dataStatus) {
		operationPositionService.updateDataStatusByNavId(list, dataStatus);
		opTemplateMapper.updateDataStatus(list, dataStatus);
	}

	@Override
	@Transactional
	public void updateDataStatusByGroupId(List<Long> list, Integer dataStatus) {
		opTemplateMapper.updateDataStatusByGroupId(list, dataStatus);
	}
}