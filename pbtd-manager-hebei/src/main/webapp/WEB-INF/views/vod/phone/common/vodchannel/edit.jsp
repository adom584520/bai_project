...
<div data-options="region:'center',border:false" style="padding:0px;">
	...
	<input type="hidden" id="id" name="id" />
		...
		绑定标签
				...
				<td class="tdContent">
				<select id="labels" name="labels"  style="width: 120px;" class="chzn-select">
						<option value="">--请选择--</option>
						<c:forEach var="map" items="${labellist }">
						<option value="${map.id }"<c:if test="${label eq  map.id}">selected="selected"</c:if> >${map.name }</option>
						</c:forEach>
					</select>
				</td>
			</tr>
