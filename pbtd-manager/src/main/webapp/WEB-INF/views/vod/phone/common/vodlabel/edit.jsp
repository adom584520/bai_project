...
<body class="easyui-layout" style="visibility: hidden">
	...
	<div data-options="region:'center',border:false" style="padding: 0px;">
		<form id="vodchannel" accept-charset="utf-8"
			...
			<table width="98%" cellspacing="1" cellpadding="3" class="tblMain"
				align="center">
				<tr>
					<td style="text-align: right; width: 20%" class="tdCaption"
						align="center">分类</td>
					<td class="tdContent"><select id="type" name="type"
						style="width: 120px;"
						<c:if test="${id>0}">disabled="disabled"</c:if>
						class="chzn-select">
							<c:forEach var="map" items="${labeltypelist }">
								<option value="${map.id }"
									<c:if test="${type eq  map.id}">selected="selected"</c:if>>${map.name }</option>
							</c:forEach>
					</select></td>
				</tr>
				<tr>
					<td style="text-align: right; width: 15%" style="width: 120px;"
						class="tdCaption" align="center">名称</td>
					<td class="tdContent"><input type="text" id="name" name="name"
						<%--  <c:if test="${id>0 }"> disabled="disabled" </c:if> --%> class="input text easyui-validatebox-disable"
						style="" data-options="validType:'text[1,10]',required:true" /></td>
				</tr>
				<tr>
					<td style="text-align: right; width: 15%" class="tdCaption"
						align="center">排序</td>
					<td class="tdContent"><input type="text" id="sequence"
						name="sequence" class="easyui-numberbox" style="width: 60px"
						data-options="min:1,max:999,required:true" /></td>
				</tr>
				<tr>
		...
		别名
				...
				<td class="tdContent">
				<input type="text" id="anothername"name="anothername"<%--  <c:if test="${id>0 }"> disabled="disabled" </c:if> --%> class="input text easyui-validatebox-disable"    style="" data-options="validType:'text[1,10]',required:true" />
				</td>
			</tr>
				<tr>
					<td style="text-align: right; width: 15%" class="tdCaption"
						align="center">状态</td>
					<td class="tdContent"><select id="status" name="status"
						style="width: 120px;" class="chzn-select">
				...
				<tr>
					<td style="text-align: right; width: 15%" class="tdCaption"
						align="center">说明</td>
					<td class="tdContent"><input type="text" id="bz" name="bz"
						class="input text easyui-validatebox-disable" style=""
						data-options="validType:'text[1,127]'" /></td>
				</tr>
			</table>
		</form>
	</div>
	...
	//页面初始化
   	$(function(){
	        ...
	        //加载数据
	        t.domain('load', {
	            url: '${pageContext.request.contextPath}/vod/phone/Vodlabel/load/' + id,
	            names: [],
	    ...
	    var  url= '${pageContext.request.contextPath}/vod/phone/Vodlabel/'+($.query.getId()=="0"?'create':'edit/'+id);  
		     t.domain('edit', {
	             title: ' ',
	             url: url,
	             data: data,
