        ...
        <script type="text/javascript"> 
            function getinfo() {
                ...
                if(typeof($.data(t[0], 'datagrid')) == 'undefined') {
                	...
                	t.domain('datagrid', {
                        title: '绑定数据',
                    	url: '${pageContext.request.contextPath}/vod/phone/Vodchannel/albuminfopage',
	                    ...
	                    title:"添加节目",
	               		...
	               		handler: function() {
	               			$(parent).domain("openDialog", { 
	    	     	        	iconCls: "icon-view", 
	    	     	        	title: "添加", 
	    	     	       		src: "${pageContext.request.contextPath}/vod/phone/vodSpecial/addalbuminfo/"+$tagsids+"?parentCode="+$parentCode+"&channelName="+$channelName+"&levels="+$levels,
	    	     	        	width: 1250, 
	    	    	        	height: 650,
	    	     	        	onClose: function() { 
	    	     	        		var ids = top.$.data(top.document.body, "choose.ids");
	    	     	        		$.ajax({
	    	                           	url:"${pageContext.request.contextPath}/vod/phone/Vodchannel/addalbuminfo",
	    	                           	type:"GET",
	    	                           	dataType:"JSON",
	               	    	...
	               	    	$.messager.confirm('确认','确认删除？',function(r){
	               	    		if(r){
	    	               	        ...
	    	               	        for (var i = 0; i < nodes.length; i++) {
	       	     	        		...
	       	     	        		$.ajax({
	       	                           	url:"${pageContext.request.contextPath}/vod/phone/Vodchannel/delalbuminfo",
	       	                           	type:"GET",
	       	                           	dataType:"JSON",
	               		...
	               		handler: function() {
	        				...
	        				nodes = $table.datagrid("getChanges");
	               	        ...
	               	        for (var i = 0; i < nodes.length; i++) {
	            	            ...
	            	            channelCode+=","+nodes[i]["channelCode"];
	            	        }
	    	     	        		$.ajax({
	    	                           	url:"${pageContext.request.contextPath}/vod/phone/Vodchannel/updatealbumsequence",
	    	                           	type:"GET",
	    	                           	dataType:"JSON",
	    	                           	data:{albumid:ids.substr(1,ids.length),channelCode:channelCode.substr(1,channelCode.length),sequence:sequence.substr(1,sequence.length)},
	    	                           	async:false,
	    	                           	success:function(){
	    	                           	 top.$.messager.alert("信息", "操作成功！", "info", null, 2000);
	    	                           		$("#vodSpecial").datagrid("load",{});               		
