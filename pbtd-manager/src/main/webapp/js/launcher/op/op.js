...
function openAddOrEdit(index) {
	...
	if (index) {
		...
		$("#paramKey").textbox("setValue",row.paramKey);
		$("#paramValue").textbox("setValue",row.paramValue);
	...
	$.ajax({
		...
		success: function(data){
			if(data.success){
				...
				$("#paramKey").textbox("setValue",obj.paramKey);
				$("#paramValue").textbox("setValue",obj.paramValue);
