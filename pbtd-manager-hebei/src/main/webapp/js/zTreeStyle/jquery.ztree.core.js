    ...
    var settings = {}, roots = {}, caches = {},
        ...
        view = {
            ...
            makeDOMNodeIcon: function (html, setting, node) {
                ...
                var clitype = node.type;
                var flag="";
                if(node.flag=="1" || node.flag==1){
                	flag= "<span style='color: red'>*</span>"
                }
                var statuscss="";//上线下线样式
                //TODO
                if(clitype == 'phone'){
                	if(node.status==0){
                		statuscss="style='color: red;font-style:oblique;text-decoration:line-through'";//下线数据样式突出  
                	}
                	html.push("<span id='", node.tId, consts.id.ICON,
	                    "' title='' treeNode", consts.id.ICON, " class='", view.makeNodeIcoClass(setting, node),
	                    "' style='", view.makeNodeIcoStyle(setting, node), "'></span><span son=1 clitype='phone'  cursequence='",node.sequence,"' curpid='",node.pId,"' curid='",node.id,"' id='", node.tId, consts.id.SPAN,
	                     "' class='", consts.className.NAME,"'",statuscss,
	                    ">",flag, name, "</span>");
                }else{
                	html.push("<span id='", node.tId, consts.id.ICON,
                             "' title='' treeNode", consts.id.ICON, " class='", view.makeNodeIcoClass(setting, node),
                             "' style='", view.makeNodeIcoStyle(setting, node), "'></span><span son=1 clitype='tv' cursequence='",node.sequence,"' curpid='",node.pId,"' curid='",node.id,"' id='", node.tId, consts.id.SPAN,
