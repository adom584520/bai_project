    ...
    <form id="vodchannel"  accept-charset="utf-8" style="margin:10px; padding:0; visibility:hidden" method="post" autoTypeset="false" columnSize="2">
			...
			<c:if test="${id>0}">
		 	...
		 	序号
					...
					<td class="tdContent">
							<input type="text" id="id" name="id"  class="input text easyui-numberbox"  disabled="disabled"  style="" data-options="validType:'text[1,127]',required:true" />	
					</td>
				</tr>
			</c:if> 
			<tr>
		 ...
		 商家选择
				...
				<td class="tdContent">
				<select id="bussid"  name="bussid"   style="width: 120px;">
            		</select>
	</td>
			</tr>
			<tr>
		 ...
		 版本选择
				...
				<td class="tdContent">
				<select id="versionid"  name="versionid"   style="width: 120px;">
            		</select>
	</td>
			</tr>
...
<script type="text/javascript">
	//页面初始化
   	$(function(){
	        ...
	        //加载数据
	        t.domain('load', {
	            url: '${pageContext.request.contextPath}/live/VersionBussInfo/load/' + id,
	            names: [],
	    ...
	    var  url= '${pageContext.request.contextPath}/live/VersionBussInfo/'+($.query.getId()=="0"?'create':'edit/'+id);  
	    t.domain('edit', {
            title: ' ',
            url: url,
            data: data,
	...
	function choosechannel(){
           	$.ajax({
               	url:"${pageContext.request.contextPath}/live/BussManage/list",
               	type:"GET",
               	dataType:"JSON",
               	data:{levels:1},
               	async:false,
               	success:function(msg){
               		var options = "";
	            		for(var i=0;i<msg.rows.length;i++){
	            			options+="<option  value="+msg.rows[i].bussid+">"+msg.rows[i].name+"</option>";
	            		}
	            		$bussid.html(options);
               	},
    ...
    //选择分组
	function choosechannel1(){
           	$.ajax({
               	url:"${pageContext.request.contextPath}/live/Version/list",
               	type:"GET",
               	dataType:"JSON",
               	data:{levels:1},
               	async:false,
               	success:function(msg){
               		var options = "";
	            		for(var i=0;i<msg.rows.length;i++){
	            			options+="<option  value="+msg.rows[i].versionid+">"+msg.rows[i].versionname+"</option>";
	            		}
