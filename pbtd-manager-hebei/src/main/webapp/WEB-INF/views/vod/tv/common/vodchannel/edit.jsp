	...
	<input type="hidden" id="id" name="id" />
		...
		归属频道
				...
				<select id="parentCode" name="parentCode"  style="width: 120px;" class="chzn-select">
						 <option id="levlesbj"  style="display: none;"    value="0">--请选择--</option> 
		...
		唯一标识
				...
				<td class="tdContent">
				<input type="text" id="channelCode"name="channelCode"  <c:if test="${id >0 }">disabled="disabled"</c:if> class="easyui-numberbox"    style=""  data-options="min:10000,max:9999999999,required:true" />
				</td>
			</tr>
			<tr>
				<td style="text-align: right; width: 15%"   style="width: 120px;" class="tdCaption"  align="center">
		名称
				...
				<td class="tdContent">
				<input type="text" id="channelName"   disabled="disabled" name="channelName"   class="input text easyui-validatebox-disable"     style="" data-options="validType:'text[1,30]',required:true" />
				</td>
			</tr>
			<tr>
		...
		层级
				...
				<select id="levels" name="levels"  style="width: 120px;" class="chzn-select">
						 <option   id="levlesbj" style="display: none;"  value="1">1</option>
...
<script type="text/javascript">
	//页面初始化
   	$(function(){
	        ...
	        //加载数据
	        t.domain('load', {
	            url: '${pageContext.request.contextPath}/vod/tv/Vodchannel/load/' + id,
	            names: [],
	            onLoadSuccess: function(data, status, XHR) {
	                ...
	                var d=data.levels;
	                if(d=="1"){
	                	document.getElementById('levlesbj').style.display='';
	                }else{
	                	document.getElementById('levlesbj').style.display='none';
	                }
	    ...
	    var  url= '${pageContext.request.contextPath}/vod/tv/Vodchannel/'+($.query.getId()=="0"?'create':'edit/'+id);  
		     t.domain('edit', {
	             title: ' ',
	             url: url,
	             data: data,
