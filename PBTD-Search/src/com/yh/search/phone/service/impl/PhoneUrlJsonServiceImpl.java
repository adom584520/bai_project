...
import com.yh.search.phone.dao.PhoneUrlJsonDao;
 ...
 */
@Service // 查询所有
public class PhoneUrlJsonServiceImpl implements PhoneUrlJsonService {

	@Autowired
	private PhoneUrlJsonDao urlJsonDao;

	@Override
	public List<PhoneSubSearchItem> getSubSearch(String seriesId) throws Exception {
	...
	// 无演员,无频道
	private List<PhoneSubSearchItem> SubSearch4(String seriesId) throws Exception {
		// 创建一个SolrQuery对象。
		SolrQuery query = new SolrQuery();
		// 设置查询条件
		query.setQuery("*:*");
		...
		List<PhoneSubSearchItem> itemList = urlJsonDao.getSubSearch(query);
		Iterator<PhoneSubSearchItem> iterator = itemList.iterator();
	...
	// 无演员,有频道
	private List<PhoneSubSearchItem> SubSearch3(String categoryName, String seriesId) throws Exception {
		// 创建一个SolrQuery对象。
		SolrQuery query = new SolrQuery();
		// 设置查询条件
		query.setQuery("*:*");
	...
	// 有演员,无频道
	private List<PhoneSubSearchItem> SubSearch2(String actor, String seriesId) throws Exception {
		...
		if (itemList.size() >= 10) {
			...
			// 需要指定默认搜索域。
			query.set("df", "play_actor");
			List<PhoneSubSearchItem> list = urlJsonDao.getSubSearch(querys);
			Iterator<PhoneSubSearchItem> iterator = list.iterator();
			...
			Iterator<PhoneSubSearchItem> iterators = itemList.iterator();
	...
	// 有演员,有频道
	private List<PhoneSubSearchItem> SubSearch1(String actor, String categoryName, String seriesId) throws Exception {
		String[] actors = actor.split("\\|");
		// 创建一个SolrQuery对象。
		SolrQuery query = new SolrQuery();
		// 设置查询条件
		...
		if (itemList.size() >= 10) {
			...
			List<PhoneSubSearchItem> list = urlJsonDao.getSubSearch(querys);
			if (list.size() == 10) {
				Iterator<PhoneSubSearchItem> iterators = list.iterator();
				while (iterators.hasNext()) {
					if (iterators.next().getSeriesCode().equals(seriesId)) {
						iterators.remove();
					}
				...
				List<PhoneSubSearchItem> lists = urlJsonDao.getSubSearch(queryz);
				Iterator<PhoneSubSearchItem> iterator = lists.iterator();
